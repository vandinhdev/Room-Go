...
public class EsbRoutes extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        from("direct:login")
                ...
                .to("http://user-management-service:8080/api/user/auth/login?httpMethod=POST&bridgeEndpoint=true")
        ...
        from("direct:register")
                ...
                .setHeader(Exchange.CONTENT_TYPE, constant("application/json"))
                .marshal().json(JsonLibrary.Jackson)
                .to("http://user-management-service:8080/api/user/auth/register?httpMethod=POST&bridgeEndpoint=true")
                .unmarshal().json(JsonLibrary.Jackson);
        ...
        from("direct:refresh")
                ...
                .setHeader(Exchange.CONTENT_TYPE, constant("application/json"))
                .marshal().json(JsonLibrary.Jackson)
                .to("http://user-management-service:8080/api/user/auth/refresh-token?httpMethod=POST&bridgeEndpoint=true")
                .unmarshal().json(JsonLibrary.Jackson);

        from("direct:verify")
                .routeId("verify-route")
                .log("👉 [ESB] Forwarding verify email")
                .to("http://user-management-service:8080/api/user/auth/verify-email?httpMethod=GET&bridgeEndpoint=true")
                .unmarshal().json(JsonLibrary.Jackson);
